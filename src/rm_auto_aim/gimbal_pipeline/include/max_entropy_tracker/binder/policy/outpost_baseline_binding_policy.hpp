...
#ifndef MAX_ENTROPY_TRACKER_BINDER_POLICY_OUTPOST_BASELINE_BINDING_POLICY_HPP_
#define MAX_ENTROPY_TRACKER_BINDER_POLICY_OUTPOST_BASELINE_BINDING_POLICY_HPP_
...
struct OutpostBaselineBindingInput {
...
struct OutpostBaselineBindingOutput {
...
class OutpostBaselineBindingPolicy {
 public:
  explicit OutpostBaselineBindingPolicy(const UnifiedConfig & config);

  void reset(int init_panel_id, std::optional<double> obs_z = std::nullopt);

  OutpostBaselineBindingOutput step(const OutpostBaselineBindingInput & input);

 private:
  OutpostBaselineBindingOutput build_output(
...
#endif  // MAX_ENTROPY_TRACKER_BINDER_POLICY_OUTPOST_BASELINE_BINDING_POLICY_HPP_
