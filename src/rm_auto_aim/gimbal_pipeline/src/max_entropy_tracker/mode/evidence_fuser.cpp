...
double normalize_weights(double w_dual, double w_margin, double w_health,
                         double w_entropy, double *nd, double *nm, double *nh,
                         double *ne) {
  const double sd = std::max(0.0, w_dual);
...
ModeEvidence EvidenceFuser::fuse(
    double timestamp, int obs_count, bool has_2dz_signature, double entropy_norm,
    double max_prob, double candidate_margin,
  ...
  const double certainty_strength = clamp01(1.0 - ev.entropy_norm);
  ...
  normalize_weights(cfg_.w_dual, cfg_.w_margin, cfg_.w_health, cfg_.w_entropy,
                    &nw_dual, &nw_margin, &nw_health, &nw_entropy);
  ...
  double enter = (1.0 - clamp01(cfg_.jump_event_weight)) * structured_strength +
                 clamp01(cfg_.jump_event_weight) * jump_event_strength;
  if (has_2dz_signature) {
    enter += std::max(0.0, cfg_.signature_2dz_bonus);
  }
