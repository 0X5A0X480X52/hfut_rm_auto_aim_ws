...
OutpostTrackerBaseline::OutpostTrackerBaseline(const UnifiedConfig &config, double dt,
...
int OutpostTrackerBaseline::infer_init_panel(const ObservationData &obs) const {
...
int OutpostTrackerBaseline::semantic_from_panel(int panel_id) const {
...
void OutpostTrackerBaseline::sync_runtime_from_backend(
    const outpost_baseline::BackendStateSnapshot &snap) {
...
void OutpostTrackerBaseline::reset_warmup() {
  warmup_active_ = config_.outpost.baseline_warmup_enable;
  warmup_frames_ = 0;
  warmup_current_group_ = -1;
  warmup_groups_.clear();
}

void OutpostTrackerBaseline::update_warmup_evidence(const ObservationData &obs) {
  if (!warmup_active_) return;
  ++warmup_frames_;
  if (warmup_frames_ > config_.outpost.baseline_warmup_max_frames) {
    warmup_frames_ = 1;
    warmup_current_group_ = -1;
    warmup_groups_.clear();
  }
  ...
  if (!start_new_group) {
    ...
    start_new_group =
        z_jump >= config_.outpost.baseline_warmup_z_jump_gate ||
        yaw_jump >= config_.outpost.baseline_warmup_yaw_jump_gate ||
        xyz_jump >= config_.outpost.baseline_warmup_xyz_jump_gate;
  }
...
OutpostTrackerBaseline::WarmupCommit OutpostTrackerBaseline::try_commit_warmup() const {
  WarmupCommit out;
  if (!warmup_active_ ||
      static_cast<int>(warmup_groups_.size()) <
          config_.outpost.baseline_warmup_min_groups) {
    return out;
  }
  ...
  for (int i = 0; i < static_cast<int>(warmup_groups_.size()); ++i) {
    if (warmup_groups_[i].sample_count >=
        config_.outpost.baseline_warmup_min_samples_per_group) {
  ...
  const double merge_gate =
      std::max(0.015, 0.6 * config_.outpost.baseline_warmup_z_jump_gate);
  ...
  if (dz02 < config_.outpost.baseline_warmup_min_large_diff) return out;
  ...
  if (ratio < config_.outpost.baseline_warmup_ratio_min ||
      ratio > config_.outpost.baseline_warmup_ratio_max || balance > 1.8) {
    return out;
  }
...
bool OutpostTrackerBaseline::run_warmup_update(const ObservationData &obs) {
  update_warmup_evidence(obs);

  outpost_baseline::BackendUpdateHint hint;
  hint.panel_id = 1;
  ...
  outpost_baseline::PublishStateInput pub_input;
  ...
  ambiguous_backend_.update(obs, outpost_baseline::BackendUpdateHint{panel, 0.5, true});
  return true;
}

void OutpostTrackerBaseline::initialize(const std::vector<ObservationData> &obs,
                                  double /*r1*/, double /*r2*/,
                                  double /*dza*/) {
  if (obs.empty()) {
    throw std::invalid_argument("OutpostTrackerBaseline requires one observation");
  }
  const ObservationData *selected =
      obs_frontend_.select_primary_observation(obs, ctx_);
  if (selected == nullptr) {
    throw std::invalid_argument("OutpostTrackerBaseline cannot select observation");
  }
  ...
  ctx_ = outpost_baseline::OutpostRuntimeContext{};
  ...
  outpost_baseline::BindingCandidate candidate;
...
void OutpostTrackerBaseline::predict(std::optional<double> target_time) {
...
bool OutpostTrackerBaseline::update(const std::vector<ObservationData> &obs) {
  outpost_baseline::BindingCandidate candidate;
  binder::BinderOutput binder_out;
  mode::ModeDecision mode_decision;
  ...
  binder_out = binder_bridge_.step(*selected, static_cast<int>(obs.size()),
                                   candidate, ctx_);
  ...
  if (ctx_.mode == mode::TrackMode::STRUCTURED) {
    ...
    outpost_baseline::BackendUpdateHint shadow = hint;
...
Eigen::Vector3d OutpostTrackerBaseline::get_center_position() const {
  return ctx_.center_pos;
}

double OutpostTrackerBaseline::get_yaw() const {
...
std::pair<double, double> OutpostTrackerBaseline::get_radii() const {
  return {radius_, radius_};
}

SpinFilterInterface &OutpostTrackerBaseline::spin_filter() {
  return structured_backend_.ukf();
}

const SpinFilterInterface &OutpostTrackerBaseline::spin_filter() const {
  return structured_backend_.ukf();
}

ManeuverResult OutpostTrackerBaseline::assess_maneuver() const {
...
Eigen::Vector3d OutpostTrackerBaseline::get_publish_velocity() const {
  return ctx_.publish_vel;
}

bool OutpostTrackerBaseline::is_ambiguous_single_mode() const {
  return ctx_.mode == mode::TrackMode::AMBIGUOUS;
}

int OutpostTrackerBaseline::effective_num_armors() const {
  return is_ambiguous_single_mode() ? 1 : 3;
}

double OutpostTrackerBaseline::confidence_scale() const {
  if (!is_ambiguous_single_mode()) return 1.0;
  return clamp01(config_.outpost.single_mode_confidence_scale);
}

std::vector<geometry_msgs::msg::Pose>
OutpostTrackerBaseline::build_armors_offset_for_message() const {
  return output_adapter_.build_armors_offset_for_message(ctx_);
}

void OutpostTrackerBaseline::refresh_debug(
    const ObservationData *obs, const outpost_baseline::BindingCandidate &candidate,
