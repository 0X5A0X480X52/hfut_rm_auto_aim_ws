...
void OutpostBinderBridge::reset(int init_panel_id, std::optional<double> obs_z) {
  ...
  debug_.valid = true;
  debug_.decoder_name = "OutpostBaselineBindingPolicy";
  debug_.id_binder_name = "BaselineHypothesisBinding";
  debug_.scorer_name = "BaselineOutpostScores";
}

binder::BinderOutput OutpostBinderBridge::step(
    const ObservationData & obs, int obs_count, BindingCandidate & candidate,
    const OutpostRuntimeContext & ctx) {
  binder::BinderOutput out;

  binder::OutpostBaselineBindingInput in;
  in.obs = &obs;
