...
Norm4TrackerBaseline::Norm4TrackerBaseline(const UnifiedConfig &config, double dt,
                                         bool /*enable_oscillation*/)
    : BaseTracker(dt),
      config_(config),
      backend_(norm4_baseline::create_backend(
          norm4_baseline::backend_type_from_string(
              config_.norm4_baseline.backend_config.backend_type),
          config, dt)),
      maneuver_detector_(config.maneuver) {}

void Norm4TrackerBaseline::initialize(const std::vector<ObservationData> &obs,
  ...
  const auto &warmup_cfg = config_.norm4_baseline.warmup;
  bool use_warmup = warmup_cfg.enable_dual_seed_01 && !obs[0].panel_id.has_value();

  if (use_warmup) {
    init_warmup(obs, r1, r2, dza);
    set_mode(norm4_baseline::Norm4BaselineMode::AMBIGUOUS);
  } else {
...
void Norm4TrackerBaseline::predict(std::optional<double> target_time) {
...
bool Norm4TrackerBaseline::update(const std::vector<ObservationData> &obs) {
  ...
  std::vector<norm4_baseline::MeasurementEval> evals;
  evals.reserve(hypotheses.size());

  for (const auto &hyp : hypotheses) {
    norm4_baseline::MeasurementEval eval;
    if (hyp.kind == norm4_baseline::HypothesisKind::Single) {
  ...
  const auto &sel_cfg = config_.norm4_baseline.hypothesis_selector;
  int topk_count = std::max(1, sel_cfg.topk);
  std::vector<norm4_baseline::TopKEntry> topk;
  ...
  bool allow_commit = (mode_ == norm4_baseline::Norm4BaselineMode::STRUCTURED);
  if (!allow_commit) {
    ...
    if (commit_gate_pass) {
      ...
      norm4_baseline::UkfTrial trial;

      if (best_hyp.kind == norm4_baseline::HypothesisKind::Single) {
...
Eigen::Vector3d Norm4TrackerBaseline::get_center_position() const {
  return backend_->spin_filter().get_center_position();
}

double Norm4TrackerBaseline::get_yaw() const {
  return backend_->spin_filter().get_yaw();
}

std::pair<double, double> Norm4TrackerBaseline::get_radii() const {
  return backend_->spin_filter().get_radii();
}

SpinFilterInterface &Norm4TrackerBaseline::spin_filter() {
  return backend_->spin_filter();
}

const SpinFilterInterface &Norm4TrackerBaseline::spin_filter() const {
  return backend_->spin_filter();
}

ManeuverResult Norm4TrackerBaseline::assess_maneuver() const {
...
Eigen::Vector3d Norm4TrackerBaseline::get_publish_velocity() const {
  const auto &sf = backend_->spin_filter();
...
bool Norm4TrackerBaseline::is_ambiguous_single_mode() const {
  return mode_ == norm4_baseline::Norm4BaselineMode::AMBIGUOUS;
}

int Norm4TrackerBaseline::effective_num_armors() const { return 4; }

double Norm4TrackerBaseline::confidence_scale() const {
  return (mode_ == norm4_baseline::Norm4BaselineMode::AMBIGUOUS) ? 0.7 : 1.0;
}

std::vector<geometry_msgs::msg::Pose>
Norm4TrackerBaseline::build_armors_offset_for_message() const {
  return {};
}

void Norm4TrackerBaseline::select_topk(
    std::vector<norm4_baseline::MeasurementEval> &evals,
  ...
  for (int i = 0; i < k; ++i) {
    weights[i] /= logsumexp;
    norm4_baseline::TopKEntry entry;
...
void Norm4TrackerBaseline::populate_debug_snapshot() {
  debug_snapshot_.valid = last_hypothesis_debug_.valid;
  debug_snapshot_.track_mode = (mode_ == norm4_baseline::Norm4BaselineMode::AMBIGUOUS) ? 1 : 0;
...
void Norm4TrackerBaseline::init_warmup(const std::vector<ObservationData> &obs,
                                       double r1, double r2, double dza) {
  warmup_state_ = norm4_baseline::WarmupState{};
...
bool Norm4TrackerBaseline::run_warmup(const std::vector<ObservationData> &obs) {
  const auto &warmup_cfg = config_.norm4_baseline.warmup;
...
void Norm4TrackerBaseline::promote_warmup_winner() {
  ...
  set_mode(norm4_baseline::Norm4BaselineMode::STRUCTURED);
...
void Norm4TrackerBaseline::set_mode(norm4_baseline::Norm4BaselineMode m) {
  mode_ = m;
  apply_mode_routing();
}

void Norm4TrackerBaseline::apply_mode_routing() {
  const auto &routing = config_.norm4_baseline.mode_routing;
