...
std::shared_ptr<CompositeProcessModel> create_baseline_process_model(
...
Norm4UkfBackendBaseline::Norm4UkfBackendBaseline(const UnifiedConfig &config, double dt)
    : config_(config),
      dt_(dt),
      process_model_(create_baseline_process_model(config)),
...
void Norm4UkfBackendBaseline::reset(const ObservationData &obs, int panel_id,
...
void Norm4UkfBackendBaseline::predict(double dt) {
...
PredictContext Norm4UkfBackendBaseline::buildPredictContext() const {
  PredictContext ctx;
  ctx.x_prior = x_;
  ctx.P_prior = P_;
  ctx.k_prior = k_;
...
std::string Norm4UkfBackendBaseline::r_type_for_panel(int panel_id) {
  return (panel_id % 2 == 0) ? "r1" : "r2";
}

std::string Norm4UkfBackendBaseline::armor_layer_for_panel(int panel_id) {
  return (panel_id % 2 == 0) ? "lower" : "upper";
}

Eigen::Vector4d Norm4UkfBackendBaseline::obs_model_single(
...
void Norm4UkfBackendBaseline::generate_sigma_points(
...
MeasurementEval Norm4UkfBackendBaseline::evaluateSingle(
  ...
  const auto &baseline = config_.norm4_baseline.ukf_baseline;
  double sp = baseline.sigma_pos_xy;
  double sz = baseline.sigma_pos_z;
  double sy = baseline.sigma_yaw;
  Eigen::Matrix4d R =
  ...
  const auto &gt = config_.norm4_baseline.ukf_baseline.gate;
  bool gate_pass = true;
...
MeasurementEval Norm4UkfBackendBaseline::evaluateDual(
  ...
  double sz = baseline.sigma_pos_z;
  double sy = baseline.sigma_yaw;
  double r_scale = baseline.dual_raw_R_scale;
...
UkfTrial Norm4UkfBackendBaseline::tryUpdateSingle(
    const PredictContext &ctx, const ObservationData &obs,
    int panel_id) const {
  UkfTrial trial;
  const int p = ((panel_id % 4) + 4) % 4;
  ...
  const auto &su = config_.norm4_baseline.ukf_baseline.single_update;
...
UkfTrial Norm4UkfBackendBaseline::tryUpdateDual(
    const PredictContext &ctx, const ObservationData &obs0,
    const ObservationData &obs1, int panel_id_0, int panel_id_1) const {
  UkfTrial trial;
  ...
  auto idx = state_idx_;
  const auto &du = config_.norm4_baseline.ukf_baseline.dual_update;
...
void Norm4UkfBackendBaseline::commit(const UkfTrial &trial) {
...
Eigen::Vector3d Norm4UkfBackendBaseline::get_center_position() const {
  auto idx = state_idx_;
  return Eigen::Vector3d(x_(idx.X()), x_(idx.Y()), x_(idx.Z()));
}

std::pair<double, double> Norm4UkfBackendBaseline::get_radii() const {
  auto idx = state_idx_;
  return {x_(idx.R1()), x_(idx.R2())};
}

double Norm4UkfBackendBaseline::get_dza() const {
  return x_(state_idx_.DZA());
}

double Norm4UkfBackendBaseline::get_yaw() const {
  return normalize_angle(x_(state_idx_.DELTA()));
}

double Norm4UkfBackendBaseline::get_delta() const {
  return x_(state_idx_.DELTA());
}

// ── Private helpers ──

bool Norm4UkfBackendBaseline::check_posterior_sanity(
    const Eigen::VectorXd &x_prior, const Eigen::VectorXd &x_post,
    const Eigen::MatrixXd &P_post) const {
  auto idx = state_idx_;
  const auto &ps = config_.norm4_baseline.ukf_baseline.posterior_sanity;
...
double Norm4UkfBackendBaseline::compute_reconstruction_error(
...
void Norm4UkfBackendBaseline::apply_state_constraints() {
  auto idx = state_idx_;
