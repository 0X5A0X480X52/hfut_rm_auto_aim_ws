...
: Node("armor_detector", options) {
  ...
  pnp_refiner_mode_ = this->declare_parameter("pnp_refiner.mode", std::string("none"));
  ...
  processing_timer_ = this->create_wall_timer(
    std::chrono::milliseconds(1), std::bind(&ArmorDetectorNode::processLatestFrame, this));
  createImageSub();
...
void ArmorDetectorNode::processImage(const sensor_msgs::msg::Image::ConstSharedPtr &img_msg) {
  FYT_DEBUG(
    "armor_detector", "Image frame_id: {}, odom_frame: {}", img_msg->header.frame_id, odom_frame_);
  ...
  try {
    ...
    auto odom_to_gimbal = tf2_buffer_->lookupTransform(
      odom_frame_, img_msg->header.frame_id, target_time, tf2::durationFromSec(0.01));
    extract_rotation(odom_to_gimbal);
  } catch (tf2::ExtrapolationException &ex) {
    FYT_WARN(
      "armor_detector", "TF at image stamp not cached, falling back to latest: {}", ex.what());
    try {
      auto odom_to_gimbal =
        tf2_buffer_->lookupTransform(odom_frame_, img_msg->header.frame_id, tf2::TimePointZero);
      extract_rotation(odom_to_gimbal);
...
void ArmorDetectorNode::createImageSub() {
  ...
  tf2_img_filter_ = std::make_shared<tf2_ros::MessageFilter<sensor_msgs::msg::Image>>(
    *img_sub_,
    *tf2_buffer_,
    odom_frame_,
    1,
