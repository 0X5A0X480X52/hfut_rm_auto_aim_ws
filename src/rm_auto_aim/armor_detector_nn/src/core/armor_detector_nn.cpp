...
ArmorDetectorNN::ArmorDetectorNN(const DetectorConfig &config) : config_(config) {}
...
bool ArmorDetectorNN::initialize() {
  ...
  // 3. Decode strategy
  decode_strategy_ = DecodeStrategyFactory::create();
  if (!decode_strategy_) {
    FYT_ERROR("armor_detector_nn", "Failed to create RobotPilots decode strategy");
    return false;
  }
  ...
  FYT_INFO("armor_detector_nn",
           "Core pipeline initialized with {} backend",
...
std::vector<FrameDetections> ArmorDetectorNN::detectBatch(
  ...
  for (size_t i = 0; i < images.size(); ++i) {
    ...
    // Preprocess
    {
      ...
      FYT_DEBUG("armor_detector_nn",
                "Preprocessing completed in {:.2f} ms",
      ...
      // Infer
      {
        ...
        FYT_DEBUG("armor_detector_nn",
                  "Decoding completed in {:.2f} ms, {} raw detections",
        ...
        std::vector<RawDetection> nms_result;
        {
          ...
          nms_result = applyNMS(
            raw, config_.postprocess.nms_threshold, config_.postprocess.class_agnostic_nms);
        }
        auto t_nms_end = std::chrono::steady_clock::now();
        FYT_DEBUG("armor_detector_nn",
                  "NMS completed in {:.2f} ms, {} detections remaining",
                  std::chrono::duration<double, std::milli>(t_nms_end - t_detect_end).count(),
                  nms_result.size());
        for (const auto &rd : nms_result) {
          FYT_DEBUG("armor_detector_nn",
                    "NMS candidate: class_id={} confidence={}",
                    rd.class_id,
                    rd.confidence);
        }
        ...
        for (const auto &rd : nms_result) {
          ...
          ad.center = cv::Point2f(rd.bbox.x + rd.bbox.width / 2, rd.bbox.y + rd.bbox.height / 2);
        ...
        if (static_cast<int>(fd.detections.size()) > config_.postprocess.max_detections) {
          std::partial_sort(fd.detections.begin(),
        ...
        FYT_DEBUG("armor_detector_nn",
                  "Pose estimation completed in {:.2f} ms, {} detections published",
  ...
  FYT_DEBUG("armor_detector_nn", "Batch detection completed: {} frames processed", results.size());
  return results;
}
