...
void OpenVINOBackend::load(const BackendConfig &config) {
  ...
  if (model_path.empty()) {
    throw std::runtime_error("OpenVINOBackend: model_path is empty");
  }
  if (!std::filesystem::is_regular_file(model_path)) {
    throw std::runtime_error("OpenVINOBackend: model file does not exist: " + model_path);
  }
  ...
  if (ext_pos != std::string::npos) {
    ...
    std::transform(model_ext.begin(), model_ext.end(), model_ext.begin(), [](unsigned char c) {
  ...
  FYT_INFO("armor_detector_nn", "OpenVINOBackend loading: {} on {}", model_path, device);
  ...
  info_.precision = is_int8_quantized_                    ? "int8"
                    : config.precision == Precision::FP16 ? "fp16"
                                                          : "fp32";

  loaded_ = true;
  FYT_INFO(
    "armor_detector_nn", "OpenVINOBackend loaded successfully (precision: {})", info_.precision);
}
...
std::vector<TensorOutput> OpenVINOBackend::infer(const TensorInput &input) {
...

  {
    ...
    if (num_elements != input.host_data.size()) {
      throw std::runtime_error("OpenVINOBackend::infer: input size mismatch. Expected " +
  ...
  for (size_t i = 0; i < output_names_.size(); ++i) {
    ...
    std::memcpy(out.host_data.data(), out_tensor.data<float>(), num_elements * sizeof(float));
...
BackendInfo OpenVINOBackend::info() const { return info_; }
...
void OpenVINOBackend::validateModelIO() {
  if (input_shape_.size() != 4) {
    throw std::runtime_error("OpenVINOBackend: input must be 4D (NCHW)");
  }
  if (input_shape_[0] != 1) {
    FYT_WARN("armor_detector_nn", "OV input batch dim is {}, expected 1", input_shape_[0]);
  }
  ...
  try {
    ...
    if (rt_info.count("nncf")) {
      ...
      if (nncf.count("version")) {
        ...
        FYT_INFO("armor_detector_nn", "NNCF quantization version: {}", ver);
      }
    }
  } catch (...) {
    // rt_info is optional
...
std::string OpenVINOBackend::selectAvailableDevice(const std::string &preferred_device) {
  ...
  std::transform(upper_pref.begin(), upper_pref.end(), upper_pref.begin(), ::toupper);
  for (const auto &d : available) {
    std::string upper = d;
...
ov::Tensor OpenVINOBackend::makeInputTensor(const TensorInput &input) const {
  ...
  if (expected_elements != input.host_data.size()) {
    throw std::runtime_error("OpenVINOBackend::makeInputTensor: input size mismatch. Expected " +
  ...
  if (expected_type == ov::element::f32) {
    ...
    std::memcpy(tensor.data<float>(), input.host_data.data(), expected_elements * sizeof(float));
    return tensor;
  }
